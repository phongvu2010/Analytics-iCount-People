# app/main.py
# Để chạy quy trình ETL: python -m app.cli run-etl
# Để chạy web server: python -m app.cli serve
# Bạn cũng có thể thay đổi host và port: python -m app.cli serve --host 0.0.0.0 --port 8888
# Để xem tất cả các lệnh có sẵn: python -m app.cli --help
import logging
...
# Cấu hình logging ngay khi module được tải.
# Điều này rất quan trọng để Uvicorn có thể ghi log đúng cách, kể cả khi reload.
...
# Thêm các sự kiện startup và shutdown (good practice)
@api_app.on_event("startup")
async def startup_event():
    logging.info("FastAPI application startup...")
    # Trong tương lai, bạn có thể khởi tạo các kết nối CSDL cho API tại đây

@api_app.on_event("shutdown")
async def shutdown_event():
    logging.info("FastAPI application shutdown.")
    # Đóng các kết nối CSDL tại đây

# Định nghĩa các endpoint
@api_app.get("/", summary="Endpoint chào mừng")
def read_root():
    """
    Endpoint chào mừng của API.
    """
    return {"message": "Chào mừng đến với iCount-People API"}

# Trong tương lai, bạn có thể thêm các router khác từ Giai đoạn 2 ở đây
# from app.api.routers import traffic
# api_app.include_router(traffic.router, prefix="/api/v1/traffic")
